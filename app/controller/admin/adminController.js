...
const addHospital = async (req, res) => {
    try {
        const { ownerName, socialMediaLinks = '', name = '', email, mobileNumber = '', address = '', latitude = '', longitude = '', content = '[]', password = '', city, type, specializationDescription, specialization } = req.body;
        ...
        const hospitalData = { ownerName, socialMediaLinks, name, email, mobileNumber, address, city, type, specialization, specializationDescription, latitude, longitude, password: md5(password) };
        if (profile.length != 0) {
...
//                 Model: hospitalModel,
//                 condition,
//                 fields: 'ownerName socialMediaLinks name email mobileNumber address profile content city type specializationDescription',
//                 limit: parseInt(limit),
//                 offset: parseInt(offset),
//                 sortBy: { create: -1 }
//             });
...
const getAllSpecializations = async (req, res) => {
    try {
        ...
        // Flatten and get unique specializations
        const allSpecializations = [
            ...new Set(
                hospitals
                    .flatMap(hospital => hospital.specialization || [])
                    .map(spec => spec.trim().toLowerCase()) // Normalize if needed
            )
        ];
...
const getHospitals = async (req, res) => {
    try {
        ...
        // Apply specialization filter (supports exact or regex match)
        if (specialization && specialization != null) {
            condition.specialization = { $regex: new RegExp(specialization, 'i') }; // Case-insensitive partial match
        }
        ...
        if (latitude !== null && longitude !== null) {
            const hospitals = await hospitalModel.aggregate([
                ...
                { $limit: parseInt(limit) },
                {
                    $project: {
                        ...
                        specializationDescription: { $ifNull: ["$specializationDescription", ""] }
                    }
                }
            ]);
            ...
            const { data: hospitals, totalRecords } = await selectdatawithjoin({
                Model: hospitalModel,
                condition,
                fields: 'ownerName socialMediaLinks name email mobileNumber address profile content city type specialization specializationDescription',
                limit: parseInt(limit),
                offset: parseInt(offset),
                sortBy: { create: -1 }
            });
            ...
            for (let h = 0; h < hospitals.length; h++) {
                const media = await mediaModel.find({
                    delete: false,
                    type: 'HOSPITAL',
                    typeId: hospitals[h]['_id']
                });
                hospitals[h]['media'] = media;

                const contentArr = await contentModel.find({
                    delete: false,
                    hospitalId: hospitals[h]['_id']
                });
                hospitals[h]['contentDetails'] = contentArr;
            }
...
const getAppointmentsWithDetails = async (req, res) => {
    try {
        const { appointmentId, dateFilter, status, hospitalId, userId } = req.body;
        ...
        const pipeline = [
            ...
            // },
            {
                $match: hospitalId || userId ? {
                    ...(hospitalId ? { 'appointmentData.hospitalId': new mongoose.Types.ObjectId(hospitalId) } : {}),
                    ...(userId && userId != "0" ? { 'appointmentData.userId': new mongoose.Types.ObjectId(userId) } : {})
                } : {}
            },
            {
                $lookup: {
